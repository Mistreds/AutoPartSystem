        ...
        public ReactiveCommand<Unit, Unit> CreateDayReport => ReactiveCommand.Create(() => {
            ...
            using (ExcelPackage package = new ExcelPackage(newFile))
            {
                ...
                if (openCloseCashList == null)
                {
                    ...
                    var marz_emps = db.MarzhEmployee.Include(p => p.Invoice).ThenInclude(p => p.GoodsInvoice).ThenInclude(p => p.Goods).ThenInclude(p => p.Warehouse).Include(p => p.Invoice.Employee).ThenInclude(p => p.City).Where(p => p.Invoice.Date >= day_start && p.Invoice.Date <= day_end && p.EmployeeId == EmpDay).ToList();
                    foreach (var mar in marz_emps)
                    {
                        int j1 = 1;
                        foreach (var good in mar.Invoice.GoodsInvoice)
                        {
                            sheet.Cells[$"A{i}"].Value = j1;
